...
class give_data:
    def __init__(self):
        ...
        self.tickers = ["AAPL", "VIX.INDX", 'SPY', 'XAUUSD.OANDA', 'BCO.ICMTRADER','NVDA']
    ...
    def get_data(self,eval=False):
        ...
        query=self.net_functions.create_query(self.tickers[4],self.table_name)
        ...
        if eval:
            train_data=data['bco_icmtrader_close']
            ...
            image_tensor=self.net_functions.make_tensor(data,image_tensor,self.tickers[4],result=True)
        ...
        labels=torch.tensor(labels,dtype=torch.float32)
        image_tensor=torch.tensor(image_tensor)
        image_tensor=self.net_functions.make_tensor(data,image_tensor,self.tickers[4])
